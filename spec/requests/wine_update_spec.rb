...
RSpec.describe "Wine update flow", type: :request do
  ...
  let!(:wine) { producer.wines.create!(name: "Spec Wine #{rand(100000)}", region: "South Australia", color: "Red") }
  ...
  it "updates a wine via its slug URL" do
    ...
    patch wine_path(wine.slug), params: { wine: { region: "South Australia" } }
    expect(response).to redirect_to(wine_path(wine.slug))
    expect(wine.reload.region).to eq("South Australia")
  end
